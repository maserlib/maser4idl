PRO read_wind_tnr_l2_block,lu,cycle_tnr,t_ur8,freq,dbagc,sdbv,dbagc2,sdbv2


;+
; NAME:
;       read_wind_tnr_l2_block
;
; PURPOSE:
; 	Read a block of a Wind Waves tnr level 2 data
;       file.
;
; CATEGORY:
;	I/O
...
;
; CALLING SEQUENCE:
;	read_wind_tnr_l2_block,lu,cycle_tnr,t_ur8,freq,dbagc,sdbv,dbagc2,sdbv2
;
; INPUTS:
;       lu - Logical unit number of the opened tnr data file.
;	
; OPTIONAL INPUTS:
;       None.
;
...
;
; OUTPUTS:
;	cycle_tnr - Structure array containing key parameters for the
;                   cycles of the current block.
;       t_ur8     - Time values (ur8 format).
;       freq      - Frequency values (in kHz).
;       dbagc     - AGC values in dB
;       sdbv      - Spectra values in microVolts^2/Hz
;       dbagc2    - TNR B AGC values in dB (if mode 4)
;       sdbv2     - TNR B Spectra values in microVolts^2/Hz (if mode 4)
;
; OPTIONAL OUTPUTS:
;       None.
;		
...
if (n_params() lt 1) then begin
   ...
   print,'read_wind_tnr_l2_block,lu,cycle_tnr,t_ur8,freq,dbagc,sdbv,dbagc2,sdbv2
   return
endif
