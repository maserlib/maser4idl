FUNCTION read_wind_waves_df,file,header, $
...
;
; PURPOSE:
; 	Read a Wind Waves rad1 l3 direction-finding data
;	file.
;
; CATEGORY:
;	I/O
...
;
; CALLING SEQUENCE:
;       data = read_wind_waves_df( file, header)
;
; INPUTS:
...
if (n_params() lt 1) then begin
   ...
   print,'data = read_wind_waves_df(file,header, $'
...
if (VERBOSE) then begin
   message,file+':',/info
   message,string(format='(I6," sweeps, ",I6," data samples.")', $
                  n_sweep,n_data),/info
endif
...
data = replicate({data_wind_waves_df},n_data)
header = replicate({header_wind_waves_df},n_sweep)
...
for i=0l,n_sweep-1l do begin 
   ...
   data(ipos:ipos+Ndata_i-1l).receiver_code = header_tmp.receiver_code
