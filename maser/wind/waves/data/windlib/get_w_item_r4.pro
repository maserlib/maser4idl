...
;
; PURPOSE:
; 		get Wind/Waves real4 item values reading the WindLib
...
;
; CALLING SEQUENCE:
;		values = get_w_item_r4(receiver_code, item, date=date)
;            or 
;               values = get_w_item_r4(receiver_code, item, lz_filename=lz_filename)
; INPUTS:
...
;
; KEYWORD PARAMETERS:
;               /HOURS   - Returns the time in decimal hours for the day.
...
if (n_params() lt 2) or (date_flag+lz_flag eq 0) then begin
   ...
   print,'values = get_w_item_r4(receiver_code, item, $'
...
if not (lz_flag) then lz_filename=filepath
...
while (ur8_time le ur8_stop) do begin
   ...
   else if (HOURS) then event_scet_i=float(event_scet_i - fix(event_scet_i))*24.0
                    

   ;Get item's value for the current event
   value_i = fltarr(dsize)
   ok=w_item_r4(ch,item,value_i,dsize,ret_size)
   if (ok ne 1) then value_i[*] = nan_value

   time=[time,float(event_scet_i)]
   values=[values,float(value_i)]
   sweep=[sweep,i]
   i++
endwhile
