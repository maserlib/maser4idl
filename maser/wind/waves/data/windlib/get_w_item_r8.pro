FUNCTION get_w_item_r8,receiver_code, item, $
...
;
; PURPOSE:
; 		get Wind/Waves real 8b item values reading the WindLib
...
;
; CALLING SEQUENCE:
;		values = get_w_item_r8(receiver_code, item, date=date)
;            or 
;               values = get_w_item_r8(receiver_code, item, lz_filename=lz_filename)
; INPUTS:
...
if (n_params() lt 2) or (date_flag+lz_flag eq 0) then begin
   ...
   print,'values = get_w_item_r8(receiver_code, item, $'
...
if not (keyword_set(nan_value)) then nan_value = !values.d_nan
...
while (ur8_time le ur8_stop) do begin
   ...
   value_i = dblarr(dsize)
   ok=w_item_r8(ch,item,value_i,dsize,ret_size)
   if (ok ne 1) then value_i[*] = nan_value

   time=[time,event_scet_i]
   values=[values,double(value_i)]
   sweep=[sweep,i]
   i++
endwhile
